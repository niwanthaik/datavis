...
@RestController
@RequestMapping("/")
public class PassengersPerAgent {

    @RequestMapping(value = "/chart/passengersperagent", method = RequestMethod.GET)
    public void passengersPerAgent() {
        MongoClient mongo = new MongoClient();
        DB db = mongo.getDB("datarepo");

        DBCollection collection = db.getCollection("rawdata");

        DBObject group = new BasicDBObject("$group", new BasicDBObject("_id", "$AGENT_NAME")
                .append("passengers", new BasicDBObject("$sum", 1)));
